...
app.config["SECRET_KEY"] = "83ASDFcvjkla%2933#asdAoB"

app.config["OPTS"] = opts

## Init Cartoonizer and load its weights
wb_cartoonizer = WB_Cartoonize(
    os.path.abspath("white_box_cartoonizer/saved_models/"), opts["gpu"]
)
    ...
    if flask.request.method == "POST":
        try:
            if flask.request.files.get("image"):
                filename = flask.request.files["image"].filename.split(".")[0]
                print()
                ...
                img_name = "cartoonized_" + str(filename)
...
def convert_image():
    ...
    if flask.request.files.get("image"):
        ...
        # cartoon_image = wb_cartoonizer.infer(file)
        print(file)
        ...
        f = open(cartoonized_img_name, 'rb')
        myfile = f.read()
        f.close()
